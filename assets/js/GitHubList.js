function listrepos(username, listelement, showProfile=false, showPagesHome=false, exclude=[]) {
    return new Promise((resolve, reject) => {
      ...
      const reposToShow = ["Ecommerce-Django", "youtube_DW", "kedro-mlflow", "chat-with-youtube-video", "quiz_app_tkinter", "pipeline-dashboard"]
      fetch(reposurl).then(res => res.json()).then((out) => {
        ...
        for (let i = 0;; i++) {
          ...
          if (reposToShow.includes(out[i].name)) {
